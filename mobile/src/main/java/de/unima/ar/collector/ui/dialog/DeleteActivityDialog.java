...
public class DeleteActivityDialog extends DialogFragment
{
    ...
    public Dialog onCreateDialog(Bundle savedInstanceState)
    {
        ...
        final ArrayAdapter subacadapter = new ArrayAdapter(this.getActivity(), android.R.layout.simple_spinner_item, sActivities);
        ...
        builder.setPositiveButton("Delete", new DialogInterface.OnClickListener()
        {
            ...
            {
                // Before we can delete the activity we need to delete activity data and all subactivities
                ...
                SQLDBController.getInstance().delete(SQLTableName.ACTIVITYDATA, "activityid = ?", new String[]{ "" + aId });
                SQLDBController.getInstance().delete(SQLTableName.SUBACTIVITIES, "activityid = ?", new String[]{ "" + aId });

                // Delete the activity
                SQLDBController.getInstance().delete(SQLTableName.ACTIVITIES, "id = ?", new String[]{ "" + aId });

                Toast.makeText(context, "Activity " + activities.get((int) sp.getSelectedItemId()).name + " successfull deleted!", Toast.LENGTH_LONG).show();
