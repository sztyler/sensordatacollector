 ...
 */
public class RotationVectorSensorCollector extends SensorCollector
{
    ...
    public RotationVectorSensorCollector(Sensor sensor)
    {
        ...
        for(String device : devices) {
            RotationVectorSensorCollector.createNewPlotter(device);
        }
    }
    ...
    public void SensorChanged(float[] values, long time)
    {
        ...
        RotationVectorSensorCollector.updateLivePlotter(deviceID, values);
    }
    ...
    public static void createNewPlotter(String deviceID)
    {
        ...
        levelPlot.rangeMin = -1;
        levelPlot.rangeMax = 1;
        levelPlot.rangeName = "rotation";
        levelPlot.SeriesName = "Rotation Vector";
        levelPlot.domainName = "Axis";
        levelPlot.domainValueNames = Arrays.copyOfRange(valueNames, 0, 3);
        levelPlot.tableName = SQLTableName.ROTATION;
        levelPlot.sensorName = "Rotation Vector";


        PlotConfiguration historyPlot = new PlotConfiguration();
        historyPlot.plotName = "HistoryPlot";
        historyPlot.rangeMin = -1;
        historyPlot.rangeMax = 1;
        historyPlot.domainMin = 0;
        historyPlot.domainMax = 50;
        historyPlot.rangeName = "rotation";
        historyPlot.SeriesName = "Rotation Vector";
    ...
    public static void updateLivePlotter(String deviceID, float[] values)
    {
        ...
        if(plotter == null) {
            RotationVectorSensorCollector.createNewPlotter(deviceID);
    ...
    {
        String sqlTable = "CREATE TABLE IF NOT EXISTS " + SQLTableName.PREFIX + deviceID + SQLTableName.ROTATION + " (id INTEGER PRIMARY KEY, " + valueNames[3] + " INTEGER, " + valueNames[0] + " REAL, " + valueNames[1] + " REAL, " + valueNames[2] + " REAL)";
    ...
    {
        DBUtils.flushCache(SQLTableName.ROTATION, cache, deviceID);
    }
}