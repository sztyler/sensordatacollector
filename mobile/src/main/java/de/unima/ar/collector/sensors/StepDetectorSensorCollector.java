...
public class StepDetectorSensorCollector extends SensorCollector
{
    ...
    public StepDetectorSensorCollector(Sensor sensor)
    {
        ...
        for(String device : devices) {
            StepDetectorSensorCollector.createNewPlotter(device);
        }
    }
    ...
    public void SensorChanged(float[] values, long time)
    {
        ...
        StepDetectorSensorCollector.updateLivePlotter(deviceID, values);
    }
    ...
    public static void createNewPlotter(String deviceID)
    {
        ...
        levelPlot.tableName = SQLTableName.STEP;
        levelPlot.sensorName = "Step Detector";
    ...
    public static void updateLivePlotter(String deviceID, float[] values)
    {
        ...
        if(plotter == null) {
            StepDetectorSensorCollector.createNewPlotter(deviceID);
    ...
    {
        String sqlTable = "CREATE TABLE IF NOT EXISTS " + SQLTableName.PREFIX + deviceID + SQLTableName.STEP + " (id INTEGER PRIMARY KEY, " + valueNames[1] + " INTEGER UNIQUE, " + valueNames[0] + " REAL)";
