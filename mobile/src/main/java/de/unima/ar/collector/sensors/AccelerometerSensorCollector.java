...
public class AccelerometerSensorCollector extends SensorCollector
{
    ...
    private static Map<String, FE>             features = new HashMap<>();
    ...
    public AccelerometerSensorCollector(Sensor sensor)
    {
        ...
        // create new plotter / new features
        ...
        for(String device : devices) {
            ...
            AccelerometerSensorCollector.createNewFeatueres(device);
        }
    }
    ...
    public void SensorChanged(float[] values, long time)
    {
        ...
        AccelerometerSensorCollector.updateLiveFeatures(deviceID, time, values);
    }
    ...
    //    @Override
    public FE getFeature(String deviceID)
    {
        return features.get(deviceID);
    }
    ...
    public static void updateLiveFeatures(String deviceID, long time, float[] values)
    {
        FE feature = features.get(deviceID);
        if(feature == null) {
            AccelerometerSensorCollector.createNewFeatueres(deviceID);
            feature = features.get(deviceID);
        }

        feature.addAccelerationData(time, values, null, null, null, null);
    }


    public static void createDBStorage(String deviceID)
    {
        String sqlTable = "CREATE TABLE IF NOT EXISTS " + SQLTableName.PREFIX + deviceID + SQLTableName.ACCELEROMETER + " (id INTEGER PRIMARY KEY, " + valueNames[3] + " INTEGER UNIQUE, " + valueNames[0] + " REAL, " + valueNames[1] + " REAL, " + valueNames[2] + " REAL)";
        SQLDBController.getInstance().execSQL(sqlTable);

        String sqlTable2 = "CREATE TABLE IF NOT EXISTS " + SQLTableName.PREFIX + deviceID + SQLTableName.ACCELEROMETER + "_features (id INTEGER PRIMARY KEY, windowID INTEGER, start INTEGER, end INTEGER, cc REAL, energy REAL, entropy REAL, iqr REAL, kurtosis REAL, mad REAL, mean REAL, meandc REAL, median REAL, sd REAL, variance REAL)";
        SQLDBController.getInstance().execSQL(sqlTable2);
    }
    ...
    private static void createNewPlotter(String deviceID)
    {
    ...
    private static void createNewFeatueres(String deviceID)
    {
        features.put(deviceID, new FE(de.unima.ar.collector.features.model.Sensor.ACCELERATION));
    }
}