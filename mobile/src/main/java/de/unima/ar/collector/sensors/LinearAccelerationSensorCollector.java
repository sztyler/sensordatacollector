 ...
 */
public class LinearAccelerationSensorCollector extends SensorCollector
{
    ...
    public LinearAccelerationSensorCollector(Sensor sensor)
    {
        super(sensor);
        ...
        for(String device : devices) {
            LinearAccelerationSensorCollector.createNewPlotter(device);
        }
    }
    ...
    public void SensorChanged(float[] values, long time)
    {
        ...
        LinearAccelerationSensorCollector.updateLivePlotter(deviceID, values);
    }
    ...
    public static void createNewPlotter(String deviceID)
    {
        ...
        levelPlot.SeriesName = "Linear Acceleration";
        levelPlot.domainName = "Axis";
        levelPlot.domainValueNames = Arrays.copyOfRange(valueNames, 0, 3);
        levelPlot.tableName = SQLTableName.LINEAR;
        levelPlot.sensorName = "Linear Acceleration";
        ...
        historyPlot.SeriesName = "Linear Acceleration";
    ...
    {
        String sqlTable = "CREATE TABLE IF NOT EXISTS " + SQLTableName.PREFIX + deviceID + SQLTableName.LINEAR + " (id INTEGER PRIMARY KEY, " + valueNames[3] + " INTEGER, " + valueNames[0] + " REAL, " + valueNames[1] + " REAL, " + valueNames[2] + " REAL)";
    ...
    {
        DBUtils.flushCache(SQLTableName.LINEAR, cache, deviceID);
    }
}